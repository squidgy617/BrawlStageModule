...
class stSlipspaceShadeInterface {
public:
    enum State {
        State_Inactive = 0x0,
    ...
    stSlipspaceShadeInterface() : playerId(-1), followPlayerId(-1), isRecord(false), currentFrame(0), state(State_Inactive) {};
    stSlipspaceShadeInterface(int playerId, int followPlayerId) : playerId(playerId), followPlayerId(followPlayerId), isRecord(false), currentFrame(0), state(State_Inactive) {};
    ...
    void update(float deltaFrame) {
        ...
        if (g_ftManager->isFighterActivate(entryId, -1) && g_ftManager->isFighterActivate(followEntryId, -1)) {
            ...
            //fighter->m_moduleAccesser->getGroundModule().canIntercept(soGroundShapeImpl::Correct_None, 0);
...
template <u32 L>
class stSlipspaceShade : public stSlipspaceShadeInterface {
protected:
    int infoLength;
...
public:
    ...
    stSlipspaceShade(int playerId, int followPlayerId) : stSlipspaceShadeInterface(playerId, followPlayerId), infoLength(L) {
...
template <u32 L>
class stSlipspaceGhost : public stSlipspaceShade<L> {
    gfFileIOHandle handle;

public:
    stSlipspaceGhost(int playerId, int followPlayerId) : stSlipspaceShade<L>(playerId, followPlayerId){
